...
test("Verify Error Handling for Mandatory Checkout Fields",async({page,homePage,shopPage,cartPage,checkoutPage}) =>{
    ...
    await expect(checkoutPage.validationErrorMessages).toBeVisible();
    await expect(checkoutPage.validationErrorMessages).toContainText("Billing Street address");
});